...
void Copter::update_land_detector()
{
    ...
    if (!motors->armed()) {
        ...
        bool descent_rate_low = fabsf(inertial_nav.get_velocity_z()) < 10;

        // if we have a healthy rangefinder only allow landing detection below 2 meters
        bool rangefinder_check = (!rangefinder_alt_ok() || rangefinder_state.alt_cm_filt.get() < LAND_RANGEFINDER_MIN_ALT_CM);

        //previous// if (motor_at_lower_limit && accel_stationary && descent_rate_low && rangefinder_check) {
        if (accel_stationary && descent_rate_low && rangefinder_check) {
